...
class AuditTests(unittest.TestCase):
    ...
    def test_inspector_uses_v200_independent_prompt_boxes_and_provider_payload(self):
        ...
        self.assertIn("element.title = value == null ? 'Pool data unavailable' : Number(value).toLocaleString('en-US')", html)
        self.assertIn("agDesc.innerText = `Path: ${data.gemini.path || 'Detected in PATH'}`;", html)
        self.assertNotIn("agDesc.innerText = `Path: ${data.antigravity.path || 'Detected in PATH'}`;", html)

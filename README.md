...
- **إضافة حساب جديد بسهولة:**
  ```bash
  ag add /path/to/credentials.json
  ```
  أو ببساطة بوضع ملف بيانات الحساب بالرقم التالي (مثلاً `3`) داخل المجلد:
  `~/.antigravity-accounts/3`

### 2. حسابات Codex (ChatGPT):
- **عرض الحسابات:** `cx` أو `cx list`
- **التبديل لحساب:** `cx 3` أو `c3`
- **فحص الاستهلاك:** `cx usage` أو `cusage`
- **إضافة حساب جديد بسهولة:**
  ```bash
  cx add /path/to/auth.json
  ```
  أو ببساطة بوضع ملف المصادقة بالرقم التالي داخل المجلد:
  `~/.codex-accounts/4`
